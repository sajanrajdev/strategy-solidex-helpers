...
def test_are_you_trying(sett_id):
    ...
    ## The fee is in the helper
    if sett_id == "sexWftmLp":
      assert harvest.events["PerformanceFeeGovernance"][0]["token"] == strategy.solidHelperVault()
    else:
      assert harvest.events["PerformanceFeeGovernance"][0]["token"] == strategy.sexHelperVault()
...
def test_fee_configs(sett_id):
    ...
    if sett_id == "sexWftmLp":
      ...
      assert harvest.events["PerformanceFeeGovernance"][0]["token"] == strategy.sexHelperVault()
    assert harvest.events["PerformanceFeeGovernance"][1]["token"] == strategy.want()
